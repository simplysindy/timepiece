...
def run_scraping_pipeline(config: DictConfig, targets: List[WatchTarget]) -> Dict[str, bool]:
...
def main(config: DictConfig) -> None:
    ...
    try:
        ...
        if pipeline_config.run_scraping:
            ...
            successful_scrapes = sum(1 for success in scraping_results.values() if success)
            logger.info(f"Scraping completed: {successful_scrapes}/{len(scraping_results)} successful")
