...
from .browser import BrowserManager, check_cloudflare_challenge, check_page_loaded_successfully
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if attempt == self.max_retries - 1:
                    logger.error(f"Failed to navigate after {self.max_retries} attempts: {e}")
                    return False
        ...
        for i, (brand, brand_watches) in enumerate(brand_groups.items()):
            ...
            for watch in brand_watches:
                try:
                    ...
                    if success:
                        logger.info(f"✅ Successfully processed {watch.brand} {watch.model_name}")
                    else:
                        logger.warning(f"❌ Failed to process {watch.brand} {watch.model_name}")
                
                except Exception as e:
                    logger.error(f"❌ Error processing {watch.brand} {watch.model_name}: {e}")
